...
app.use(loginMiddleware, userMiddleware, activityMiddleware)
...
mongoose
  ...
  .then(() => {
    app.listen(process.env.CANVA_BACKEND_PORT, () => {
      ...
      const checker = startActivityChecker();
...
// These next ones are the mongodb related function😃
...
const startActivityChecker = () => {
  ...
  const runCheck = async () => {
    try {
      await ActivityService.checkUserActivity();
    } catch (error) {
      console.error('Activity checker error:', error);
    }
  };
  ...
  // Set up interval
  return setInterval(runCheck, CHECK_INTERVAL);
};

async function activityMiddleware(req, res, next) {
  try {
    if (req.user && req.user._id) {
      await ActivityService.updateUserActivity(req.user._id)
    }
    next()
  } catch (error) {
